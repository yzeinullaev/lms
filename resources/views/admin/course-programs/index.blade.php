...
@section('title', 'Программы курса')

@section('content_header')
    <h1>Программы курса: {{ $parentData->getName->$lang }}</h1>
@stop
@php
...
@section('content')
    ...
    <a href="{{ url('/admin/course-programs/create/' . $parentData->id) }}" class="btn btn-success btn-sm" title="Добавить курс">
    ...
    <form method="GET" action="{{ url('/admin/course-programs/course/' . $parentData->id) }}" accept-charset="UTF-8"
        ...
        <table class="table">
            ...
            @foreach($data as $item)
                <tr>
                    ...
                    <td>
                        <a href="{{ url('/admin/course-programs/' . $item->id . '/edit/' . $parentData->id) }}" title="Редактировать курс">
                            <button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o"
                                                                      aria-hidden="true"></i> Редактировать
                            </button>
                        </a>

                        <form method="POST" action="{{ url('/admin/course-programs/' . $item->id) }}"
