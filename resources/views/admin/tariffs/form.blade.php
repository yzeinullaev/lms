        ...
        <div class="tab-pane fade @if ($loop->first) show active @endif" id="{{ $item->slug }}" role="tabpanel"
            ...
            <div class="form-group {{ $errors->has('name.' . $item->slug) ? 'has-error' : ''}}">
                <label for="name_{{ $item->slug }}"
                       class="control-label">{{ 'Наименование тарифа ' . $item->slug }}</label>
...
<div class="form-group {{ $errors->has('discount') ? 'has-error' : ''}}">
    <label for="discount" class="control-label">{{ 'Скидка' }}<p class="small">(продления курса после истечения срока)</p></label>
    <input class="form-control" name="discount" type="text" id="discount"
           value="{{ isset($data->discount) ? $data->discount : old('discount') }}" required>
    {!! $errors->first('discount', '<p class="help-block">:message</p>') !!}
</div>

<div class="form-group {{ $errors->has('addition_price') ? 'has-error' : ''}}">
    <label for="addition_price" class="control-label">{{ 'Доп. сумма' }}<p class="small">(Если переключится на другой тариф)</p></label>
    <input class="form-control" name="addition_price" type="text" id="addition_price"
           value="{{ $data->addition_price ?? old('addition_price') }}" required>
    {!! $errors->first('addition_price', '<p class="help-block">:message</p>') !!}
</div>

<div class="form-group {{ $errors->has('soon') ? 'has-error' : ''}}">
    <label for="soon" class="control-label">{{ 'Активен скоро' }}<p class="small">(кнопка на фронте)</p></label>
    <select class="form-control" name="soon" id="soon">
        <option value="1" {{ isset($data->soon) && $data->soon == 1 ? 'selected' : ''}}>Активен</option>
        <option value="0" {{ isset($data->soon) && $data->soon == 0 ? 'selected' : ''}}>Не активен</option>
    </select>
