...
@section('content')
    ...
    <a href="{{ url('/admin/tariffs/create') }}" class="btn btn-success btn-sm" title="Добавить триф">
        <i class="fa fa-plus" aria-hidden="true"></i> Добавить
    </a>

    <form method="GET" action="{{ url('/admin/tariffs') }}" accept-charset="UTF-8"
        ...
        <table class="table">
            ...
            @foreach($data as $item)
                <tr>
                    ...
                    <td>
                        <a href="{{ url('/admin/tariff-items/tariff/' . $item->id) }}" title="Значение">
                            <button class="btn btn-secondary btn-sm">
                                <i class="fa fa-pencil-square-o" aria-hidden="true"></i> Значение
                            </button>
                        </a>

                        <a href="{{ url('/admin/tariffs/' . $item->id . '/edit') }}" title="Edit">
                            <button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o"
                                                                      aria-hidden="true"></i> Редактировать
                            </button>
                        </a>

                        <form method="POST" action="{{ url('/admin/tariffs' . '/' . $item->id) }}"
