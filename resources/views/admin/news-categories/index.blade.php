...
@section('content')
    ...
    <a href="{{ url('/admin/news-categories/create') }}" class="btn btn-success btn-sm" title="Добавить категорию">
        <i class="fa fa-plus" aria-hidden="true"></i> Добавить
    </a>

    <form method="GET" action="{{ url('/admin/news-categories') }}" accept-charset="UTF-8"
        ...
        <table class="table">
            ...
            @foreach($data as $item)
                <tr>
                    ...
                    <td>{{ $item->getNews->count() }}</td>
                    <td>@if($item->is_active) Активен @else Не активен @endif</td>
                    <td>
                        <a href="{{ url('/admin/news-categories/news/' . $item->id) }}" title="Edit">
                            <button class="btn btn-secondary btn-sm">
                                <i class="fa fa-pencil-square-o" aria-hidden="true"></i> Значение
                            </button>
                        </a>

                        <a href="{{ url('/admin/news-categories/' . $item->id . '/edit') }}" title="Edit">
                            <button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o"
                                                                      aria-hidden="true"></i> Редактировать
                            </button>
                        </a>

                        <form method="POST" action="{{ url('/admin/news-categories' . '/' . $item->id) }}"
